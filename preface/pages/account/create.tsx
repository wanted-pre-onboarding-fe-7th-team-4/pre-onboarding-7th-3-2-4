...
import useCreateNewAccount from "components/accountDetail/hook/useCreateNewAccount";

import styled from "styled-components";
import { ACCOUNT_STATUS, BROKERS } from "lib/constants/constants";
import { TAccountStatusKey } from "lib/utils/account/changeAccountStatusFromNumberToKorean";
import { TBrokersKey } from "lib/utils/account/changeBrokerCodeToKorean";
import Layout from "components/Layout/dashboard/Layout";
import { generateNewAccount } from "lib/utils/account/generateNewAccount";

const CreateAccount = () => {
  const [status, setStatus] = useState<TAccountStatusKey>(9999);
      ...
      <form onSubmit={handleNewAccount}>
        <FormItem>
          ...
          <select
            ...
            value={status}
            onChange={(e) => {
              setStatus(Number(e.currentTarget.value) as TAccountStatusKey);
            }}
          >
...
CreateAccount.getLayout = function getLayout(page: ReactElement) {
  return <Layout title="계좌목록">{page}</Layout>;
};
